...
class LoginForm(forms.Form):
    ...
    password = forms.CharField(
        widget=forms.PasswordInput(
            attrs={'class': 'form-control', 'id': 'InputPassword', 'placeholder': _('Пароль от аккаунта')}),
        max_length=30,
...
class QuestionForm(forms.ModelForm):
    ...
    tags = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control', 'id': 'InputTags',
                                                         'placeholder': _(
                                                             'Ключевые слова, относящиеся к теме вопроса')}),
                           max_length=50,
                           label='Теги', required=True, help_text=_('Максимальное количество тегов: 3'))
...
class AnswerForm(forms.ModelForm):
    text = forms.CharField(
        widget=forms.Textarea(attrs={'class': 'form-control border-secondary', 'id': 'InputText',
                                     'placeholder': _('Введите текст ответа'), 'rows': 5}), max_length=2000,
        min_length=10,
